            ...
            <form action="{{ route('admin.logistik.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('nama_item') is-invalid @enderror" 
                           id="nama_item" name="nama_item" value="{{ old('nama_item') }}" 
                           placeholder="Cth: Galon Air, Plastik Sampah" required>
                    @error('nama_item')
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('stok_awal') is-invalid @enderror" 
                           id="stok_awal" name="stok_awal" value="{{ old('stok_awal', 0) }}" required>
                    @error('stok_awal')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('satuan') is-invalid @enderror" 
                           ...
                           placeholder="Cth: pcs, galon, roll" required>
                    @error('satuan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
