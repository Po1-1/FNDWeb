            ...
            <form action="{{ route('admin.mahasiswa.update', $mahasiswa) }}" method="POST">
                ...
                <div class="row g-3">
                        ...
                        <select class="form-select @error('kelompok_id') is-invalid @enderror" id="kelompok_id" name="kelompok_id" required>
                            ...
                            @foreach ($kelompoks as $kelompok)
                                <option value="{{ $kelompok->id }}" 
                                    {{ old('kelompok_id', $mahasiswa->kelompok_id) == $kelompok->id ? 'selected' : '' }}>
                                    {{ $kelompok->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control @error('no_urut') is-invalid @enderror" id="no_urut" name="no_urut" value="{{ old('no_urut', $mahasiswa->no_urut) }}" required>
                        @error('no_urut') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-6 align-self-center">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" id="is_vegan" name="is_vegan" value="1" {{ old('is_vegan', $mahasiswa->is_vegan) ? 'checked' : '' }}>
                ...
                @error('alergis') <div class="alert alert-danger">{{ $message }}</div> @enderror
                <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="form-check">
                                <input class="form-check-input" 
                                       ...
                                       value="{{ $alergi->id }}" 
                                       id="alergi-{{ $alergi->id }}"
                                       {{-- Cek 'old' (jika validasi gagal) ATAU data asli dari controller --}}
                    ...
                    @empty
                        <div class="col-12">
                            <p class="text-muted">Tidak ada data alergi di database.</p>
                        </div>
                    @endforelse
                </div>
